...
services = {
  ...
  "simple-service" = {
    ...
    name                           = "simpleservice"  # Name of the ECS service
    ignore_task_definition_changes = true            # Whether to ignore task definition changes
    ecr_repository                 = "fake-nginx-repo" # ECR repository name for the container image
    git_repository = {
      name   = "fake/nginx-service"  # Git repository name for CI/CD
      branch = "main"               # Branch to use for deployments
    }
    ...
    task_iam_role_name = "simple-service-task-role"  # IAM role name for the task
    ...
    task_exec_iam_role_name = "simple-service-task-exec"  # IAM role for task execution
    task_exec_secret_arns = [
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:*"  # Secrets the task can access
    ]
    ...
    container_definitions = {
      simple-service = {
        ...
        port_mappings = [
          {
            name          = "simple-service"  # Name of the port mapping
...
security_settings = {
  vpc_id              = "vpc-0577524a0e38786ea"  # VPC ID where services will run
